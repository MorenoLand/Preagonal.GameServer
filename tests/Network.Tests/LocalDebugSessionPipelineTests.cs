...
public sealed class LocalDebugSessionPipelineTests
{
    [Fact]
    public void ProcessLengthPrefixedLoginRequiresExplicitDevOnlyAuthOptIn()
    {
        ...
        Assert.Equal("Local debug auth must be explicitly enabled.", ex.Message);
    }
    ...
    public void ProcessLengthPrefixedLoginRunsDevOnlyAuthLoadsNwAndStopsAtSendLevelBoundary()
    {
        ...
        Assert.Equal(LocalDebugStopPoint.BeforeRuntimeWorldSimulation, result.StopPoint);
        Assert.Contains(result.Log, line => line.Contains("LOCAL DEBUG auth accepted", StringComparison.Ordinal));
        ...
        Assert.Contains(result.Log, line => line.Contains("LOCAL DEBUG socket flush used Gen5", StringComparison.Ordinal));
    }
    ...
    public void ProcessLengthPrefixedLoginReportsMissingDevLevelAsWarpFailedBoundary()
    {
        ...
        Assert.Equal(LocalDebugStopPoint.MissingLevel, result.StopPoint);
