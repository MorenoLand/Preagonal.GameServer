...
public sealed class ProductionPostLoginPacketDispatcherTests
{
    [Fact]
    public void DispatchDecodedPacketAppliesConfirmedPlayerPropsSubset()
    {
        ...
        Assert.Equal(ProductionPostLoginPacketDispatchStatus.Handled, result.Status);
    ...
    public void DispatchDecodedPacketMatchesMsgPliNullDisconnectThresholdForUnassignedPackets()
    {
        ...
        ProductionPostLoginPacketDispatchResult result = null!;
        for (var i = 0; i < 6; i++)
            result = dispatcher.DispatchDecodedPacket(Packet(25, 1, 2, 3));

        Assert.Equal(ProductionPostLoginPacketDispatchStatus.InvalidPacketLimitExceeded, result.Status);
        Assert.False(result.ContinueSession);
