...
- `GServ.Protocol.Tests`: 22 tests covering packet IDs, player type bits, Graal/raw integer encoding, encryption gen behavior, framing, bundle reading, signature and disconnect packet bytes.
- `GServ.Network.Tests`: 2 tests covering initial awaiting-login state and confirmed client prelude generation selection.
- `GServ.Core.Tests`: 1 test recording recovered `gs2lib` commit.
- `GServ.Scripting.Tests`: 1 test guarding that scripting remains intentionally unimplemented.
