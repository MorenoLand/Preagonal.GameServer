...
public sealed class LocalDebugTcpServer(IPAddress address, int port, LocalDebugSessionPipeline pipeline) : IDisposable
{
    ...
    public async Task<LocalDebugSessionResult> AcceptOneAsync(CancellationToken cancellationToken)
    {
        ...
        while (!cancellationToken.IsCancellationRequested)
        {
            ...
            try
            {
                input = await ReadOneLengthPrefixedFrame(stream, cancellationToken).ConfigureAwait(false);
            }
        ...
        CancellationToken cancellationToken)
    {
        ...
        await ReadExact(stream, payload, cancellationToken).ConfigureAwait(false);
