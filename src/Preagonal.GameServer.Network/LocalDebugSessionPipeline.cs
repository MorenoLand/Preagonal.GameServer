...
public enum LocalDebugStopPoint
{
    ...
    NwLevelFileLoader levelLoader)
{
    ...
    public LocalDebugSessionConnection CreateConnection() =>
        new(this, options.PlayerId);

    internal LocalDebugSessionResult ProcessConnectionInput(
    ...
    {
        if (!options.EnableLocalDebugAuth)
            throw new InvalidOperationException("Local debug auth must be explicitly enabled.");
        ...
        foreach (var frame in PacketFramer.ReadLengthPrefixedFrames(input))
        {
            if (session.LoginPacket is null)
            {
                if (!session.ReceiveLoginPacket(frame.Span))
                    return Finish(session, false, LocalDebugStopPoint.Rejected, log);

                RunLocalDebugLoginFlow(session, log);
            ...
            catch (NotSupportedException ex)
            {
                ...
                log.Add("Unsupported post-login frame received by local-debug shell; continuous loop stopped before gameplay/runtime packet handling.");
                return Finish(session, session.Lifecycle == SessionLifecycle.DynamicLevelPayloadSent, ToStopPoint(session), log);
            }
    ...
    private void RunLocalDebugLoginFlow(ClientSessionSkeleton session, List<string> log)
    {
        var preWorld = new PreWorldAuthBoundary(new(
            MaxPlayers: 128,
        ...
        log.Add($"LOCAL DEBUG auth accepted for {accountName}; this is not production server-list behavior.");
        ...
        var continuation = PlayerSendLoginContinuation.Begin(
            ...
            new(
                ...
                AdminIps: ["0.0.0.0"],
                IsGuest: false),
            new(OnlyStaff: false, options.ServerName, ActiveSessions: []));
        ...
        if (!loaded.Success)
        {
            WarpWorldEntryBoundary.BeginSetLevel(
                session,
                new LocalDebugLevelLookup(null),
                new(
                    options.LevelName,
                    options.StartX,
                    options.StartY,
        ...
        WarpWorldEntryBoundary.BeginSetLevel(
            session,
            new LocalDebugLevelLookup(new(loaded.LevelName)),
            new(
                loaded.LevelName,
                options.StartX,
                options.StartY,
    ...
    private static LocalDebugStopPoint ToStopPoint(ClientSessionSkeleton session) =>
        ...
        ReadOnlySpan<byte> packet)
    {
        ...
        log.Add(
            $"Applied decoded PLI_PLAYERPROPS to local-debug runtime player {session.Id}: " +
        ...
        List<string> log)
    {
        if (login.Type is PlayerSessionType.Client3 or PlayerSessionType.RemoteControl2 && login.EncryptionKey is { } key)
        {
            ...
            log.Add($"LOCAL DEBUG socket flush used Gen5 with login key {key}.");
            return;
        }
        ...
        if (login.Type == PlayerSessionType.Web)
        {
            ...
            log.Add("LOCAL DEBUG socket flush used Gen1 for web client.");
            return;
        }

        throw new NotSupportedException($"Local debug socket flush for {login.Type} is not source-confirmed yet.");
    }

    private sealed record LocalDebugLevelLookup(LevelEntrySnapshot? Level) : ILevelLookup
    {
    ...
    private sealed record LocalDebugAccount(
        ushort PlayerId,
...
public sealed class LocalDebugSessionConnection
{
    ...
    internal LocalDebugSessionConnection(LocalDebugSessionPipeline pipeline, ushort playerId)
    {
        _pipeline      = pipeline;
        _session       = new(playerId);
        _runtimePlayer = new(playerId, string.Empty, RuntimePlayerKind.Client);
    }
    ...
    public LocalDebugSessionResult ProcessLengthPrefixedInput(ReadOnlySpan<byte> input)
    {
        ...
        {
            _inboundDecoder = new(_session.InboundEncryptionGeneration, login.EncryptionKey ?? 0);
            _inboundFramer  = new(new(StripRawDataTrailingNewline: true));
        }
        return result;
    }
}
