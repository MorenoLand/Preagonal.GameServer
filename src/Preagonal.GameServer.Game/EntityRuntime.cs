...
public static class LevelItemRuntime
{
        ...
        DurablePlayerInventoryState player)
    {
        ...
        if (level.AddItem(x, y, itemType))
            return new(true, EntityPackets.ItemAdd(encodedX, encodedY, itemId), [], [], itemType);

        return new(false, [], EntityPackets.ItemDelete(encodedX, encodedY), [], itemType);
    }
        ...
        DurablePlayerInventoryState player)
    {
        ...
        if (itemType == LevelItemType.Invalid)
            return new(false, forwardPacket, [], []);
        ...
        return new(true, forwardPacket, [], playerPropsPayload, itemType);
    }
...
public sealed class RuntimeBaddy
{
    ...
    private bool TryRollDrop(ICombatRandom rng, out BaddyDropPacket packet)
    {
        ...
        {
            packet = new(CombatDropRuntime.BuildBaddyDropPacket(X, Y, itemType), X, Y, itemType);
            _droppedPackets.Add(packet);
            return true;
        }

        packet = new([], X, Y, LevelItemType.Invalid);
        return false;
    }
