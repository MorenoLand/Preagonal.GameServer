...
public sealed class Gs2PlayerProperties : ScriptProperties<Gs2PlayerContext>
{
    ...
    {
        if (args.Length > 0)
            player.Messages.Add(new(player.Account, args[0]?.GetValue()?.ToString() ?? ""));
        return 0;
    }
    ...
    {
        if (args.Length > 0)
            player.WeaponActions.Add(new(player.Account, args[0]?.GetValue()?.ToString() ?? "", true));
        return 0;
    }
    ...
    {
        if (args.Length > 0)
            player.WeaponActions.Add(new(player.Account, args[0]?.GetValue()?.ToString() ?? "", false));
        return 0;
    }
...
public sealed class Gs2ServerScriptHost
{
    ...
    public async Task<Gs2ScriptRunResult> Call(string scriptName, string eventName, params string[] args)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                return new(true, _output.Skip(start).ToArray(), _clientTriggers.Skip(triggerStart).ToArray(), _player.Messages.Skip(messageStart).ToArray(), _player.WeaponActions.Skip(weaponStart).ToArray());
            }
            finally
            {
        ...
        {
            return new(false, _output.Skip(start).ToArray(), _clientTriggers.Skip(triggerStart).ToArray(), _player.Messages.Skip(messageStart).ToArray(), _player.WeaponActions.Skip(weaponStart).ToArray(), ex.Message);
        }
    }
    ...
    private static int SendPm(Script script, IStackEntry[] args)
    {
        ...
        if (player is not null && args.Length > 1)
            player.Messages.Add(new(args[0]?.GetValue()?.ToString() ?? "", args[1]?.GetValue()?.ToString() ?? ""));
        return 0;
    }
