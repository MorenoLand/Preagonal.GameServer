...
public enum ProductionPostLoginPacketDispatchStatus
{
    Handled,
    Blocked,
    InvalidPacket,
...
public sealed record ProductionPostLoginPacketDispatchResult(
    ProductionPostLoginPacketDispatchStatus Status,
    ...
    byte[] OutboundBytes)
{
    public static ProductionPostLoginPacketDispatchResult Handled(byte rawPacketId, PlayerToServerPacketId packetId, string message) =>
        new(ProductionPostLoginPacketDispatchStatus.Handled, true, rawPacketId, packetId, message, []);

    public static ProductionPostLoginPacketDispatchResult Blocked(byte rawPacketId, string message) =>
        new(ProductionPostLoginPacketDispatchStatus.Blocked, false, rawPacketId, null, message, []);

    public static ProductionPostLoginPacketDispatchResult Invalid(byte rawPacketId, int invalidPacketCount) =>
        new(
            ProductionPostLoginPacketDispatchStatus.InvalidPacket,
            true,
            rawPacketId,
            null,
    ...
    public static ProductionPostLoginPacketDispatchResult InvalidLimitExceeded(byte rawPacketId, int invalidPacketCount) =>
        new(
            ProductionPostLoginPacketDispatchStatus.InvalidPacketLimitExceeded,
            false,
            rawPacketId,
            null,
...
public sealed class ProductionPostLoginPacketDispatcher
{
    private readonly RuntimePlayer _player;

    public ProductionPostLoginPacketDispatcher(RuntimePlayer player)
    {
        _player = player;
    }
    ...
    public ProductionPostLoginPacketDispatchResult DispatchDecodedPacket(ReadOnlySpan<byte> packet)
    {
        if (packet.IsEmpty)
    ...
    private ProductionPostLoginPacketDispatchResult DispatchPlayerProps(byte rawPacketId, ReadOnlySpan<byte> body)
    {
        ...
        return ProductionPostLoginPacketDispatchResult.Handled(
            rawPacketId,
    ...
    private ProductionPostLoginPacketDispatchResult CountInvalidPacket(byte rawPacketId)
    {
        ...
        return InvalidPacketCount > 5
            ? ProductionPostLoginPacketDispatchResult.InvalidLimitExceeded(rawPacketId, InvalidPacketCount)
            : ProductionPostLoginPacketDispatchResult.Invalid(rawPacketId, InvalidPacketCount);
    }
