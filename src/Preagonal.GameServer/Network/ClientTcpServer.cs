...
public sealed class ClientTcpServer(
	IPAddress address,
	int port,
...
) : IDisposable
{
    ...
    private readonly TcpListener _listener     = CreateListener(address, port);
    ...
    public async Task<ClientTcpSessionResult> AcceptOneAsync(CancellationToken cancellationToken)
    {
        ...
        return await RunSessionAsync(client, session, cancellationToken).ConfigureAwait(false);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        connectionRegistry?.Register(session.PlayerId, stream);
        ...
        try
        {
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                if (read == 0)
                {
                    ...
                    return new(session.PlayerId, ClientTcpSessionStopReason.ClientDisconnected, lastDiagnostic);
                }
                ...
                {
                    framesHandled++;
                    var result = await handler.HandleFrameAsync(session, frame, cancellationToken).ConfigureAwait(false);
                    if (!string.IsNullOrEmpty(result.Diagnostic))
                        lastDiagnostic = result.Diagnostic;

                    if (result.OutboundBytes.Length != 0)
                        await stream.WriteAsync(result.OutboundBytes, cancellationToken).ConfigureAwait(false);

                    if (!result.ContinueSession)
                    {
                        lastDiagnostic = AppendReadDebug(lastDiagnostic, firstReadDebug);
                        return new(session.PlayerId, ClientTcpSessionStopReason.HandlerStopped, lastDiagnostic);
                    }
                }
            }
        ...
        {
            connectionRegistry?.Unregister(session.PlayerId);
        }
    }
    ...
    private static ClientSocketSessionContext CreateSession(TcpClient client, ushort playerId)
    {
        ...
        return new(playerId, remoteAddress);
    }
