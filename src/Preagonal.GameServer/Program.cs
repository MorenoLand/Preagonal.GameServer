...
var pipeline = new LocalDebugSessionPipeline(
    new(EnableLocalDebugAuth: true, LevelName: config.LevelName),
    new(fileSystem));
            ...
            switch (packetId)
            {
                case ListServerToServerPacketId.VerifyAccount2:
                {
                    ...
                    if (result.OutboundBytes.Length != 0)
                        await clientConnections.SendAsync(result.PlayerId, result.OutboundBytes, cancellationToken).ConfigureAwait(false);
                    foreach (var broadcast in result.Broadcasts)
                    {
                        if (broadcast.OutboundBytes.Length != 0)
                        {
                            var sent = await clientConnections.SendAsync(broadcast.PlayerId, broadcast.OutboundBytes, cancellationToken).ConfigureAwait(false);
                ...
                case ListServerToServerPacketId.ServerInfo:
                    ...
                    if (warp.OutboundBytes.Length != 0)
                        await clientConnections.SendAsync(warp.PlayerId, warp.OutboundBytes, cancellationToken).ConfigureAwait(false);
