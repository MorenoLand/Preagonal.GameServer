...
| ---: | --- | --- | --- | --- | --- |
| 0 | `PLPROP_NICKNAME` | `GCHAR len` + bytes, clamped to packet bytes remaining | Applies word filter, defaults empty warned nickname to `unknown`, then `setNick`. | Adds global `PLO_OTHERPLPROPS`; echoes to self unless `FORWARDSELF` is set. | Parser consumes the confirmed string bytes, including terminal truncated payloads through `CString::readChars` semantics. Runtime word-filter, `setNick`, global/self forwarding, and nickname persistence side effects remain blocked. |
...
| 8 | `PLPROP_SWORDPOWER` | `GUChar`, optional `GCHAR len` + image clamped to packet bytes remaining | `<=4` selects default `swordN` capped by `swordlimit`; `>4` subtracts `30` and reads custom image. Old clients append `.gif` when extensionless. `Account::setSwordImage` truncates to 223. | Generic local/self forwarding via `getProp`: `GCHAR(swordPower + 30)`, `GCHAR(swordImage.length)`, image bytes. | Implemented for default `healswords=false` settings, custom-image parsing including terminal truncated payloads, truncation, and generic local forwarding. The `healswords=true` negative-power/wrap behavior remains blocked until fixture-confirmed. Loaded/global recipient routing remains blocked. |
| 9 | `PLPROP_SHIELDPOWER` | `GUChar`, optional `GCHAR len` + image clamped to packet bytes remaining | `<=3` selects default `shieldN` capped by `shieldlimit`; `>3` subtracts `10` and reads custom image. One-byte 1.41 bug path continues without mutation when no bytes remain. `Account::setShieldImage` truncates to 223. | Generic local/self forwarding via `getProp`: `GCHAR(shieldPower + 10)`, `GCHAR(shieldImage.length)`, image bytes. | Implemented for default/custom parsing including terminal truncated payloads, settings limit, 1.41 no-change bug, truncation, and generic local forwarding. Loaded/global recipient routing remains blocked. |
| 10 | `PLPROP_GANI` | old client: `GUChar`/image bytes clamped to packet bytes remaining; modern: `GCHAR len` + bytes clamped to packet bytes remaining | Old clients mutate bow power/image. Modern clients call `setGani`. | `"spin"` sends four `PLO_HITOBJECTS` packets around the player. Generic forwarding applies. | Implemented for modern gani string, terminal truncated modern payloads, plus old-client bow power/image parsing, truncated image payloads, mutation, and generic forwarding. Modern `"spin"` side-effect packets remain blocked. |
| 11 | `PLPROP_HEADGIF` | `GUChar len`, optional image bytes clamped to packet bytes remaining | `<100` default head, `>100` custom `len - 100`, trims embedded newline only when the newline index is greater than zero, so a leading newline is preserved; old clients append `.gif`; `100` means no change. `Account::setHeadImage` truncates stored images to 123. | Adds global prop when changed; generic local/self forwarding also applies. `getProp` serializes `GCHAR(headImage.length + 100) + headImage`. | Implemented for parser/runtime mutation, newline edge cases, terminal truncated custom payloads, and generic local forwarding. Loaded/global forwarding recipient routing remains blocked. |
| 12 | `PLPROP_CURCHAT` | `GCHAR len`, stores up to `223` bytes, clamped to packet bytes remaining | Sets chat, updates last-chat time, runs `processChat`, applies word filter if unprocessed. If `len > 223`, C++ reads only 223 bytes and leaves remaining bytes for following props. If the packet ends first, `CString::readChars` returns the remaining bytes. | May echo filtered chat to self; V8 sends chat to level NPCs. Generic local/self forwarding uses `GCHAR(chatMessage.length) + chatMessage`. | Implemented for parser/runtime current-message storage, truncated terminal payloads, and generic local forwarding. `m_lastChat`, `processChat`, word filter echo, and V8 NPC chat events remain blocked. |
...
| 20 | `PLPROP_CURLEVEL` | `GCHAR len` + bytes, clamped to packet bytes remaining | Non-V8 stores `m_levelName`; V8 reads/discards. | Generic forwarding via `getProp`: GMAP clients receive map name, singleplayer levels append `.singleplayer`, otherwise level name. | Implemented for parser/runtime mutation, terminal truncated payloads, plus live forwarding for normal and singleplayer level names. GMAP map-name forwarding remains blocked until the live GMAP state path is fixture-confirmed. |
...
| 24 | `PLPROP_CARRYNPC` | `GUInt` | Stores carried NPC id, with duplicate-carry ownership checks unless `duplicatecanbecarried` is true. Terminal truncated scalar reads preserve the unsigned `CString::readGUInt()` value. | May send self `PLO_PLAYERPROPS`, self/global `PLO_NPCDEL2`, and level `PLO_OTHERPLPROPS`. | Parser consumes the confirmed unsigned encoding. Runtime mutation, duplicate ownership checks, NPC deletion packets, and forwarding remain blocked on NPC/runtime ownership. |
...
| 34 | `PLPROP_ACCOUNTNAME` | `GCHAR len` + bytes, clamped to packet bytes remaining | Reads and ignores. | Generic forwarding from current `getProp(PLPROP_ACCOUNTNAME)`. | Parser/applier implemented as consume-only no-op, including terminal truncated payloads. Live forwarding uses runtime account state instead of the discarded incoming bytes. Stateless forwarding remains blocked. |
| 35 | `PLPROP_BODYIMG` | `GCHAR len` + bytes, clamped to packet bytes remaining | Calls `setBodyImage`, which stores `newImage.subString(0, 223)`. | Generic forwarding. | Implemented as runtime mutation, terminal truncated payload parsing, and generic local forwarding using the current body image string. |
...
| 37-41 | `PLPROP_GATTRIB1..5` | `GCHAR len` + bytes, clamped to packet bytes remaining | Stores `ganiAttributes[0..4]`. | Generic forwarding. | Implemented as runtime mutation, terminal truncated payload parsing, and generic local forwarding using the original string payload. |
| 42 | `PLPROP_ATTACHNPC` | `GUChar object_type` + `GUInt npcID` | Stores attached NPC id. Object type is read but ignored for state; `getProp` always serializes object type `0` because only NPC attachments are supported. Unsigned NPC ids are preserved even when they exceed signed `int`. | Explicitly appends attach prop to `levelBuff`; generic local tail is false for id 42. | Implemented for parser/runtime attached-id storage and source-confirmed outgoing payload bytes. NPC attach semantics, validation, and recipient routing remain blocked. |
...
| 46-49 | `PLPROP_GATTRIB6..9` | `GCHAR len` + bytes, clamped to packet bytes remaining | Stores `ganiAttributes[5..8]`. | Generic forwarding. | Implemented as runtime mutation, terminal truncated payload parsing, and generic local forwarding using the original string payload. |
...
| 52 | `PLPROP_PLANGUAGE` | `GCHAR len` + bytes, clamped to packet bytes remaining | Stores language string. | No local forwarding in `__sendLocal`. | Implemented as local runtime mutation, including terminal truncated payload parsing. |
...
| 54-74 | `PLPROP_GATTRIB10..30` | `GCHAR len` + bytes, clamped to packet bytes remaining | Stores `ganiAttributes[9..29]`. | Generic forwarding. | Implemented as runtime mutation, terminal truncated payload parsing, and generic local forwarding using the original string payload. |
| 75 | `PLPROP_OSTYPE` | `GCHAR len` + bytes, clamped to packet bytes remaining | Stores OS string. | No local forwarding. | Implemented as local runtime mutation, including terminal truncated payload parsing. |
...
| 82 | `PLPROP_COMMUNITYNAME` | `GCHAR len` + bytes, clamped to packet bytes remaining | Reads and ignores. | Generic forwarding from current `getProp(PLPROP_COMMUNITYNAME)`. | Parser/applier implemented as consume-only no-op, including terminal truncated payloads. Live forwarding uses runtime community-name state instead of the discarded incoming bytes. Stateless forwarding remains blocked. |
